...
SmallTools st;
NTPClient TC(ntpUDP, "europe.pool.ntp.org", 2 * 3600, 10 * 3600);  // two hours offset = 2 * 3600, updating every 10 hours
PCF8574 pcf(0x20);
...
const int timeTable[] = {
  ...
  combineHoursMinutes(12, 30),
  combineHoursMinutes(13, 15),
  combineHoursMinutes(13, 20),
  combineHoursMinutes(14, 5),
  combineHoursMinutes(14, 10),
  combineHoursMinutes(14, 55),
  combineHoursMinutes(15, 0),
  combineHoursMinutes(15, 45),
...
int minutesLeft() {
  ...
  if (leftTimer.read() >= 10000) {
    leftTimer.pause();
    leftTimer.start();
    st.print(F("Calculating minutes left"));
    if (cTime > timeTable[arraySize(timeTable) - 1]) {
      st.print(F(", but out of range of time table, max:"));
...
void displayRSSI(int8_t input = -58, uint8_t xpos = 113) {
  if (RSSITimer.read() >= 6000) {
    st.println(F("displaying RSSI"));
    RSSITimer.pause();
    RSSITimer.start();
...
void TaskDisplay(void *pvParameters) {
  for (;;) {
    ...
    myDisplay(returnTimeString(), 3, true);  // main text, what time is it
    // st.println("--returnTimeString");
    myDisplay("zbyva " + String(minutesLeft()) + "m do konce", 1);  // how many minutes left till end of class/break
